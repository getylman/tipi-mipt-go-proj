...
// Base — общие параметры для любого микросервиса.
type Base struct {
	HTTPPort        string
...
func (b *Base) IsDebug() bool { return b.Environment == "dev" || b.LogLevel == "debug" }
func (b *Base) IsProd() bool  { return b.Environment == "prod" }

// GetEnv читает строковую переменную окружения с дефолтом.
...
// GetEnvInt читает целое число из env.
...
// GetEnvDuration читает duration ("5s", "1m", "500ms").
...
var validEnvironments = map[string]bool{"dev": true, "prod": true}

// ValidateBase проверяет общие поля.
...
// LoadBase читает все поля Base из env.
func LoadBase(defaultPort string) (Base, error) {
	...
	// ShutdownTimeout читается из ENV — не захардкожен
...
// MaskPassword скрывает пароль в DSN для безопасного вывода в логах.
